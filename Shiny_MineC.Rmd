...
* `observeEvent()`: To **perform an action** in response to an event. E.g. Download a data frame when user presses action button:
```{r}
# ui
...
```
* `eventReactive()`: used to **create a calculated value** that only updates in response to an event. E.g. update calculations for a plot based on sample size inputted by the user only when the user clicks on action button "Calculate!".
